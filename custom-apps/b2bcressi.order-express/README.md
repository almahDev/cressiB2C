...
```json
  "dependencies": {
    "{{vendor}}.order-express": "0.x"
  }
```

3. Coloque o app `order-express` dentro do bloco `store.{pagename}` na seção de `children`, semelhante a seguinte estrutura de blocos:
...
Lembrando que também é possível editar as propriedades acima tanto no Site Editor quanto editando as `props` dentro do bloco do `order-express`, conforme exemplo abaixo:
