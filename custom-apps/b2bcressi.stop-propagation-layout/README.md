...
```json
  "dependencies": {
    "{{vendor}}.stop-propagation-layout": "0.x"
  }
```

3. Coloque o app `stop-propagation-layout` dentro do bloco `store.{pagename}` na seção de `children`, semelhante a seguinte estrutura de blocos:

```json
"store.{pagename}": {
  "blocks": [ ... ],
  "children": [ "stop-propagation-layout", ...]
},
```
...
Lembrando que também é possível editar as propriedades acima tanto no Site Editor quanto editando as `props` dentro do bloco do `stop-propagation-layout`, conforme exemplo abaixo:
